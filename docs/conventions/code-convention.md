...
## Perl control-plane 규칙

- Proxmox와 비슷한 경로 이름, service 이름, `/api2/json` 응답 모양을 우선합니다.
- `/proc`, `/sys`, Ollama API 같은 외부 입력은 root 경로나 endpoint를 주입 가능하게 유지합니다.
- 읽기 API와 변경 API를 분리하고, sysctl/sysfs 변경은 어떤 값을 적용했는지 응답에 남깁니다.
- Perl 모듈은 core module 의존을 우선해 초기 rootfs 패키징 부담을 줄입니다.
- daemon entrypoint는 `Getopt::Long` 기반 인자와 systemd 환경 파일을 동시에 지원합니다.
